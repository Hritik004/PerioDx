from flask import Flask, session, request, jsonify, flash, send_file
...
def api_report_pdf(report_id):
    ...
    # The clinician downloading/generating this PDF right now — shown in
    # the "Generated by" row of the summary block.
    generated_by = User.query.get(session['user_id'])
    ...
    pdf_buffer = build_report_pdf(report, patient, teeth, generated_by=generated_by)

    patient_slug = (patient.first_name or "patient").strip().replace(" ", "_")
    filename = f"PerioDx_Report_{report.id}_{patient_slug}.pdf"
