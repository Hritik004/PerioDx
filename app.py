...
# LOAD MODELS ONCE
# =========================
TEETH_MODEL = YOLO(os.path.join(MODEL_FOLDER, 'teeth_seg.pt'))
CEJ_MODEL = YOLO(os.path.join(MODEL_FOLDER, 'cej_p.pt'))
AC_MODEL = YOLO(os.path.join(MODEL_FOLDER, 'best_yolo11x_seg.pt'))
...
def analyze():
    ...
    # ── Run models ────────────────────────────────────────────────────────
    res_teeth = TEETH_MODEL(orig)[0]
    res_cej   = CEJ_MODEL(orig)[0]
    res_ac    = AC_MODEL(orig)[0]
